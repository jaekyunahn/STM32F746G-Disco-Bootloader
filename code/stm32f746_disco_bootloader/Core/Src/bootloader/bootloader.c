 ...
 */
 ...
 //static uint32_t image_sdram_pointer = (uint32_t)SDRAMADDRESS_IMAGEMEMORY;
 // 터치좌표 x,y,변화 여부
...
void bootloader_application(void)
{
	...
	// 부트로더 app loop flag
	int bootloader_app_loop_flag = 1;
	...
	if (bootconfig.autoboot == 0)
	{
		...
		//bootloader app loop
		while (bootloader_app_loop_flag)
		{
			//터치 입력 정보 받기
...
struct READ_IMAGE_INFO readBitmapFileToSdram(char* fileaddress)
{
	...
	char bitmap_file_read_buffer[BITMAP_READ_DATA_SIZE] = "";
	...
	for (y = 0; y < count; y++)
	{
		...
		memset(bitmap_file_read_buffer, 0x00, sizeof(bitmap_file_read_buffer));
		// SD Card에서 읽어들일 파일 인덱스 설정
		read_file_pointer = rev_y * (imagewide)+54;

		// SD card에서 이미지 데이터 불러오기
		fatfs_read_index_readsize(fileaddress, bitmap_file_read_buffer, read_file_pointer, sizeof(char) * (imagewide));
		...
		{
			// RGB565 type으로 변환
			pixeldata[0] = bitmap_file_read_buffer[x];
			pixeldata[1] = bitmap_file_read_buffer[x + 1];
			pixeldata[2] = bitmap_file_read_buffer[x + 2];
...
struct CURRENT_PROGRESS firmwareUpdate(void)
{
	...
	//data packet
	static int data_packet_Index = 0;
	...
	static unsigned int flash_write_start_address = 0;

	// STM32 Flash Memory 쓰기 종료 주소  - static
	static unsigned int flash_write_end_address = 0;
	...
	{
		// 패킷 끝부분 문자인 '\n'를 검색,
		// 인덱스를 0부터 검색할 때 나중에 본 데이터 패킷에 있는 데이터를 마지막 부분으로 간주 할 수도 있어 
		// 내림 차순으로 검색한다
		for (x = (PACKET_MAX_SIZE - 1); x >= 0; x--)
		{
			...
			if (fCompareFunction(incomingdata, "DATA:", 5))
			{
				...
				if ((data_packet_Index == (packet_count - 1)) && (incomingdata[last_packet_size + 6] == '\n'))
				{
					// 헤더 부분 제외한 시작 부분 인덱스
					tmpIndex = 5;
					// 임시 변수 초기화
					...
					memcpy(tmp, incomingdata + tmpIndex, sizeof(char) * (last_packet_size));
					// 패킷 오류 검사를 위한 XOR Checksum 실시
					check_sum = xor_checksum(tmp, last_packet_size);
					// 체크섬 검사가 전달 된 체크섬 데이터와 일치하면 처리
					if (check_sum == incomingdata[last_packet_size + 5])
					{
						//파일 쓰기
						write_res.will_write_count = sizeof(tmp);
						write_res.last_error_code = f_write(&filedownload, tmp, (UINT)last_packet_size, (UINT*)&write_data_size);
				...
				else if ((data_packet_Index < (packet_count - 1)) && (incomingdata[packet_size + 6] == '\n'))
				{
					// 헤더 부분 제외한 시작 부분 인덱스
					tmpIndex = 5;
					// 임시 변수 초기화
					memset(tmp, 0x00, sizeof(tmp));
					// 임시 변수에 패킷 헤더 부분 제외한 영역 복사
					memcpy(tmp, incomingdata + tmpIndex, sizeof(char) * (packet_size));
					// 패킷 오류 검사를 위한 XOR Checksum 실시
					check_sum = xor_checksum(tmp, packet_size);
					// 체크섬 검사가 전달 된 체크섬 데이터와 일치하면 처리
					if (check_sum == incomingdata[packet_size + 5])
					{
						//파일 쓰기
						write_res.will_write_count = sizeof(tmp);
						write_res.last_error_code = f_write(&filedownload, tmp, (UINT)packet_size, (UINT*)&write_data_size);
						write_res.last_error_function = 2;
						write_res.last_write_count = write_data_size;
						// reset uart rx dma
						// uart 수신은 DMA로 이루어지므로 위 데이터 패킷 처리가 끝나면 수신 버퍼 초기화, DMA 초기화를 진행 한다.
			...
			if (fCompareFunction(incomingdata, "END", 3))
			{
				...
				{
					fw_update_Mode = flash_write_preparation;
					printf("AE\n");
					break;
				}
			}
	...
	else if (fw_update_Mode == flash_write_preparation)
	{
		//erase flash
		...
		// next stage
		fw_update_Mode = flash_write_process;
	}
	else if (fw_update_Mode == flash_write_process)
	{
		...
		{
			fw_update_Mode = flash_write_completion;
		}
	}
	else if (fw_update_Mode == flash_write_completion)
	{
		// 펌웨어 업데이트 완료 알림
		printf("FW update OK\n");
	...
	// 펌웨어 업데이트 진척도 백분위 계산
	// 다운로드 현황
	if (fw_update_Mode >= flash_write_preparation)
	{
		current_res.download = 100;
	}
	else
	{
		ftmp = ((float)data_packet_Index / (float)packet_count) * 100.0f;
		current_res.download = (int)ftmp;
	}
	// Flash Memory 쓰기 현황
	if (fw_update_Mode >= flash_write_completion)
	{
